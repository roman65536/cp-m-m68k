...
EXTERN UWORD	do_phio(struct iopb *);	/* external physical disk I/O routine */
...
retry:	    dirsec = i >> 2;
//printf("%s: dirsec %d i %d  cks %d \n\r",__FUNCTION__,dirsec, i,dparmp->cks);
...
//dump(GBL.dirbufp);
		...
		{			/* checksum error! */
//printf("%s2: dirsec chsum %d  dchksum() %d \n\r",__FUNCTION__,((*p)&0xff), dchksum());
