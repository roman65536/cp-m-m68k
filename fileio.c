...
EXTERN UWORD	do_phio(struct iopb *);	/* packet disk i/o handler	*/
...
EXTERN UWORD	udiv(int, int , short * );		/* unsigned divide routine	*/
...
WORD		dirindx;	/* index into directory for *dirp */
{
	...
	{
	    do setaloc(cpm_swap(dirp->dskmap.big[i++]));
		while (i <= 7);
	}
    }
}
...
//	printf("%s11: logflag %d log_dsk %x dsknum %x\n\r",__FUNCTION__,logflag,log_dsk,dsknum);
	...
	do
	{
//	printf("%s12: %x %x\n\r",__FUNCTION__,&selpkt,selpkt.infop);
	    do_phio(&selpkt);	/* actually do the disk select	*/
//	printf("%s: %x %x %x\n\r",__FUNCTION__,&selpkt,selpkt.infop, selpkt.infop->dpbp);
//	printf("xlpt: %x Dir: %x dpbp %x \n\r",selpkt.infop->xlt,selpkt.infop->dbufp,selpkt.infop->dpbp);
	    if ( (GBL.dphp = selpkt.infop) != NULL ) break;
	} while ( ! error(3) );

//	printf("%s1: dbufp %x dpbp %x dsm %x\n\r",__FUNCTION__,(GBL.dphp), (GBL.dphp)->dpbp, &GBL);
	GBL.dirbufp = (GBL.dphp)->dbufp;
			/* set up GBL copies of dir_buf and dpb ptrs */
	GBL.parmp = (GBL.dphp)->dpbp;
    }
//	printf("%s2: logflag %d log_dsk %x dsknum %x\n\r",__FUNCTION__,logflag,log_dsk,dsknum);
   if(logflag == 1) 
    {	
 ...
 //       printf("%s3:  %d %d \n\r",__FUNCTION__,(((GBL.parmp)->drm) + 1), 4 * (((GBL.parmp)->blm) + 1) ) ;	

	i = cpm_udiv( (LONG)(((GBL.parmp)->drm) + 1), 
		  4 * (((GBL.parmp)->blm) + 1), &j);
//	i = (((GBL.parmp)->drm) + 1) / (4 * (((GBL.parmp)->blm) + 1));
//	j = (((GBL.parmp)->drm) + 1) % (4 * (((GBL.parmp)->blm) + 1)); 
...

{
	...
	{
	    i = 8;
	    do clraloc(cpm_swap(dirp->dskmap.big[--i]));
		while (i);
	}
	UNLOCK
    }
