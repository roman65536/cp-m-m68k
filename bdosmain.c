...
EXTERN UWORD	bdosrw( struct fcb *, BOOLEAN, WORD );	/* Sequential and Random disk read/write */
...
REG LONG func;		/* BDOS function number */
REG LONG info;		/* d1.w word parameter  */
...
//printf("%s: %d %x %x\n\r",__FUNCTION__,func,info,infop);

//__asm__ __volatile__ (" move.b #'4',%%d0 \n\t"
//                               " jsr _chbo \n\t" :::);
	switch (func & 0xffff)	/* switch on function number */
	{
