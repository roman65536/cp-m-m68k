...
EXTERN	UWORD	rdwrt(long , unsigned char *, short );	/* disk read/write routine		*/
...
WORD		wrdfcb;		/* boolean, fcb disk map of words	*/
{
 ...
 //printf("%s: %d %x\n\r",__FUNCTION__, fcbp->dskmap.big[index], fcbp->dskmap);
    if (wrdfcb)
	return( cpm_swap(fcbp->dskmap.big[index]) );
...
REG UWORD	block;		/* block number				*/
{
    ...
    if (wrdfcb)
	fcbp->dskmap.big[index] = cpm_swap(block);
...
//printf("%s2: ext %d mod %d \n\r",__FUNCTION__,ext,mod);
...
//printf("%s: %x  read: %d rand: %d \n\r",__FUNCTION__, fcbp, reading, random);
    bigfile = ((GBL.parmp)->dsm) & ~0xff;
    if ( ( ! reading) && (fcbp->ftype[robit] & 0x80) )
		 ro_err(fcbp,(((GBL.dphp)->dpbp))->drm);
				/* check for read-only file */
//printf("%s1: %x  cur_rec: %d \n\r",__FUNCTION__, fcbp, fcbp->cur_rec);
    if (random)
    {
...
//printf("%s3: %x  cur_rec: %d  \n\r",__FUNCTION__, fcbp, fcbp->cur_rec &0xff);
   ...
   //printf("%s: idx %d block %d \n\r",__FUNCTION__,index, block );
